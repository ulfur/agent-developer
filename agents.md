...
- Pairing/resync work is not done until you record before/after evidence: save the `/api/agent/identity` payload (degraded vs paired), hit `/api/health` with an auth token so the queue header chip data is captured, and tail `logs/progress.log` for the `Control plane config refreshed` + `E-ink sections refreshed: footer_left` entries (kick off `./scripts/eink_section_selftest.py` if you need to force the footer refresh). Reference those file paths/lines inside `logs/progress.log` so future agents can prove the queue header, API, and e-ink footer all show the paired state.
